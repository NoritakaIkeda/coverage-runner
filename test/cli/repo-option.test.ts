...
test('add function', () => {
  expect(add(2, 3)).toBe(5);
    ...
    test('should preserve cloned repository when cleanup is false', async () => {
      ...
      const clonedExists = await fs.access(result.clonedPath!).then(() => true).catch(() => false)
    ...
    test('should use custom branch when specified', async () => {
      ...
      execSync('git config user.name "Test User"', { cwd: mockRepoPath, stdio: 'ignore' })
      execSync('git config user.email "test@example.com"', { cwd: mockRepoPath, stdio: 'ignore' })
      ...
      execSync('git commit -m "Initial commit"', { cwd: mockRepoPath, stdio: 'ignore' })
      execSync('git checkout -b feature-branch', { cwd: mockRepoPath, stdio: 'ignore' })
  ...
  describe('error handling', () => {
    test('should handle network timeout errors gracefully', async () => {
      ...
      const result = await executeInClonedRepo('https://github.com/slow/repo.git', {
        outputDir: resolve(testTempDir, 'output'),
        cleanup: true,
        timeout: 1000
      })
    ...
    test('should handle insufficient disk space errors', async () => {
      ...
      const result = await executeInClonedRepo('https://github.com/test/repo.git', {
        outputDir: resolve(testTempDir, 'output'),
        cleanup: true
      })
