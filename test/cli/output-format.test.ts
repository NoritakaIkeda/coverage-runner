...
describe('Output Format Options', () => {
  ...
  const testInputFile = resolve(process.cwd(), 'test/fixtures/coverage-sample.json');
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(resolve(process.cwd(), 'test/fixtures'), { recursive: true });
  ...
  test('should support --format json option', async () => {
    ...
    expect(await fs.access(lcovFile).then(() => true).catch(() => false)).toBe(false);
    expect(await fs.access(textFile).then(() => true).catch(() => false)).toBe(false);
  });
  ...
  test('should support --format lcov option', async () => {
    ...
    expect(await fs.access(jsonFile).then(() => true).catch(() => false)).toBe(false);
    expect(await fs.access(lcovFile).then(() => true).catch(() => false)).toBe(true);
    expect(await fs.access(textFile).then(() => true).catch(() => false)).toBe(false);
  });
  ...
  test('should support --format text option', async () => {
    ...
    expect(await fs.access(jsonFile).then(() => true).catch(() => false)).toBe(false);
    expect(await fs.access(lcovFile).then(() => true).catch(() => false)).toBe(false);
    expect(await fs.access(textFile).then(() => true).catch(() => false)).toBe(true);
  });
