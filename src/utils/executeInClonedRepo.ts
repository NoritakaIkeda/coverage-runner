...
export async function executeInClonedRepo(
  ...
  try {
    ...
    clonedPath = resolve(tmpdir(), `coverage-runner-clone-${timestamp}-${randomSuffix}`)
    ...
    const runners = await detectRunners(clonedPath)
    
    if (runners.length === 0) {
      return {
        success: false,
        error: 'No test runners detected in the repository. Supported: Jest, Vitest',
        outputDir,
        clonedPath
      }
    }
    ...
    for (const runnerName of runners) {
      ...
      try {
        ...
        if (result.success && result.outputPath) {
          ...
          const coverageJsonPath = resolve(clonedPath, result.outputPath, 'coverage-final.json')
          try {
            await fs.access(coverageJsonPath)
            coverageFiles.push(coverageJsonPath)
            console.log(`✅ ${runnerName} coverage completed`)
          } catch (error) {
            console.warn(`⚠️ ${runnerName} coverage file not found at ${coverageJsonPath}`)
          }
        } else {
          console.warn(`⚠️ ${runnerName} coverage failed: ${result.stderr || 'Unknown error'}`)
        }
      } catch (error) {
        console.warn(`⚠️ ${runnerName} coverage failed: ${error instanceof Error ? error.message : String(error)}`)
      }
    }
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('ENOSPC')) {
        ...
        errorMessage = 'Failed to clone repository: invalid URL or network error'
      }
    }
    ...
    if (cleanup && clonedPath) {
      try {
        ...
        console.warn(`Warning: Failed to cleanup cloned directory: ${error instanceof Error ? error.message : String(error)}`)
      }
    }
  }
}
